...
const StatDarkCard = ({ card }) => (
  <div className="shrink-0 cursor-pointer" onMouseEnter={hoverOn} onMouseLeave={hoverOff}
    style={{ ...cardSize, padding: "20px", borderRadius: "24px", background: "#08203C", boxShadow: "7px 3px 0 0 #ECEEF0", display: "flex", flexDirection: "column", justifyContent: "space-between", transition: "transform 0.3s ease, box-shadow 0.3s ease" }}>
...
const StatLightCard = ({ card }) => (
  <div className="shrink-0 cursor-pointer" onMouseEnter={hoverOn} onMouseLeave={hoverOff}
    style={{ ...cardSize, padding: "20px", borderRadius: "24px", background: "rgba(8,32,60,0.08)", display: "flex", flexDirection: "column", justifyContent: "space-between", transition: "transform 0.3s ease, box-shadow 0.3s ease" }}>
    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
...
export default function Clients() {
  ...
  // testimonial_items[] → { client_image, organization, items, service, message }
  const cards = (() => {
    ...
    items.forEach((item, i) => {
      ...
      if (item.organization || item.items) {
        result.push({
          type:    i % 2 === 0 ? "stat-dark" : "stat-light",
          logo:    "△",
          company: item.organization || "Our Client",
          stat:    item.items        || "★★★★★",
    ...
    return result.length > 1 ? result : fallbackCards;
  })();
  ...
  return (
    <section className="w-full bg-white overflow-hidden py-16">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
          <div className="flex flex-col gap-3">
            ...
            <h2 style={{ color: "#08203C", fontFamily: '"Rethink Sans", sans-serif', fontSize: "clamp(24px,3vw,36px)", fontWeight: 800, lineHeight: "1.2", margin: 0 }}>
              {heading.includes("\n") ? heading.split("\n").map((l, i) => <span key={i}>{l}{i === 0 && <br />}</span>) : heading}
            </h2>
          </div>
      ...
      <div className="w-full overflow-hidden">
        {isLoading
          ? <div className="flex gap-10 px-5 py-8">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="shrink-0 rounded-2xl animate-pulse bg-gray-200" style={cardSize} />)}</div>
