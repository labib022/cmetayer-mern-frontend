...
export default function FAQ() {
  ...
  // data.data.home["Faq"][]
  const apiFaqs = data?.data?.home?.["Faq"];

  const displayFaqs = apiFaqs?.length ? apiFaqs : fallbackFaqs;
        ...
        <div className="w-full lg:w-115 xl:w-125 shrink-0 flex flex-col gap-6">
          ...
          {openIndex !== null && displayFaqs[openIndex] ? (
            <div className="bg-[#FAFAFA] rounded-2xl p-5 sm:p-6">
              <span style={{ fontFamily: '"Rethink Sans", sans-serif' }} className="inline-block text-xs font-semibold text-[#08203C] bg-gray-100 px-3 py-1 rounded-full mb-4">
                Answer
              </span>
